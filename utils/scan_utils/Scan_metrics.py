...
from classes.db_models.ScanDB import ScanDB
from utils.create_database import Session


def calc_scan_metrics(scan: ScanDB):
    with Session() as session:
        scan_metrics = session.query(func.count(PointDB.id),
            ...
            points_scans_table.c.scan_id == ScanDB.id,
            ScanDB.id == scan.id)).first()
...
def update_scan_in_db(updated_scan: ScanDB):
    with Session() as session:
        scan = session.query(ScanDB).get(updated_scan.id)
